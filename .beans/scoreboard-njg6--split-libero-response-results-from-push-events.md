---
# scoreboard-njg6
title: Split Libero results from app data
status: completed
type: task
priority: high
tags:
...
- Save status constructors are named and shaped like domain events (`ScoreUpdateSaved`, `ResultSaved`).
...
- The initiating admin receives both a save result and a `GameUpdated` payload that can update the same row.

Because this repo is exercising Libero design, we should not treat the current one-root response shape as fixed.

## Direction

Adjust Libero and Scoreboard so load/save results are separate from app data.

Libero should generate the result error types:
...
The generated result payloads are:
...
The success side must remain `Nil`: the result carries no domain data.
...
The arrow notation describes message sequence, not nesting. The `Result(Nil, errors)` result and `ToClient` messages are separate values.

## Acceptance Criteria

- Direct request result is decoded as `Result(Nil, List(ApiLoadError))` or `Result(Nil, List(ApiSaveError))`, not as `ToClient`.
- Successful result is `Ok(Nil)` and carries no domain data.
- `ToClient` remains the server-to-client app-data vocabulary for load data and live updates.
- `GamesLoadFailed` and similarly reused failure constructors are removed or replaced by `Error([...])` result values.
- Save responses do not echo the original request payload and do not carry updated domain objects.
- Save status constructors such as `ScoreUpdateSaved` and `ResultSaved` are removed or replaced by `Ok(Nil)`.
...
- Libero generation supports the generated result error types and generic `Result(...)` result payloads without hand-written tracer glue.
...
The result shape does not need a custom operation-status wrapper type. It should use Gleam's built-in `Result(Nil, List(error))` shape directly.

There are two generated result error types:
...
Do not add `request_id` to the public generated request or result helper surface. The client runtime and server update boundary handle result payloads as messages rather than as correlated RPC promises.

## Completed notes

Libero now generates direct result helpers and the result error types.

Scoreboard uses:

- `Result(Nil, List(ApiLoadError))` for load results
- `Result(Nil, List(ApiSaveError))` for save results
