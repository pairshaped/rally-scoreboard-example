...
// here at init time; the generated rally/effect.gleam module delegates
// to these via @external.
// Derived from Rally's client effect runtime contract.
...
export function setDarkMode(enabled) {
  document.cookie = `rally_dark_mode=${enabled ? "1" : "0"}; path=/; max-age=31536000; SameSite=Lax`;
  document.documentElement.dataset.theme = enabled ? "dark" : "light";
}

export function setLang(lang) {
  document.cookie = `rally_lang=${lang}; path=/; max-age=31536000; SameSite=Lax`;
  document.documentElement.lang = lang;
}

export function readDarkModeCookie() {
  const match = document.cookie.match(/rally_dark_mode=(\d)/);
  if (match) return match[1] === "1";
  return globalThis.matchMedia?.("(prefers-color-scheme: dark)")?.matches ?? false;
}

export function readLangCookie() {
  const match = document.cookie.match(/rally_lang=([^;]+)/);
