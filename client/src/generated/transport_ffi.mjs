...
//
// Derived from Rally's client transport template. It imports encode/decode
...
// for page-init acknowledgements. Rally never inspects tag bytes or slices
...
function handleAuthenticationError(frame) {
  ...
  if (debugEnabled()) console.error("[rally] Authentication error: authentication:forbidden");
  return true;
}
    ...
    if (frame.kind === "error") {
      const msg = frame.errors
        ? (Array.isArray(frame.errors)
            ...
            : String(frame.errors))
        : "protocol error";
      if (debugEnabled()) console.error("[rally] protocol error:", msg);
      return;
    }
