...
function debugEnabled() {
  ...
  return window.__RUNTIME_DEBUG__ === true
...
function recordMessage(direction, label, data, extra) {
  ...
  if (!window.__RUNTIME_MESSAGES__) window.__RUNTIME_MESSAGES__ = [];
  const entry = {
    t: performance.now(),
  ...
  window.__RUNTIME_MESSAGES__.push(entry);
  if (window.__RUNTIME_MESSAGES__.length > 1000) {
    window.__RUNTIME_MESSAGES__ = window.__RUNTIME_MESSAGES__.slice(-500);
  }
}
...
if (typeof window !== "undefined") {
  window.__RUNTIME_FORMAT__ = formatRaw;
}
...
export function ensureSocket(url) {
  ...
  try {
    ...
    if (typeof window !== "undefined") window.__RUNTIME_WS__ = sock;
  } catch (e) {
    clearPending();
    scheduleReconnect();
    return;
...
/**
 * Send a page init frame with route params and query params. Uses
 * request_id 0 as the init sentinel. The server initializes the page's
 * ServerModel with these params instead of requiring a separate Load
 * message. The value is a two-element array [routeParam, queryObject]
 * so the server can extract both route identity and query context.
 * @param {string} url WebSocket URL
 * @param {string} module page name
 * @param {any} params route params value
 * @param {object} [query] query params as a plain object
 */
export function send_page_init(url, module, params, query) {
  ensureSocket(url);
  const queryObj = query || {};
  const payload = toWebSocketPayload(encode_request(module, 0, [params, queryObj]));
  logFrame("->", `page_init ${module}`, params, { query: queryObj });
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(payload);
  } else {
    pendingSends.push(payload);
