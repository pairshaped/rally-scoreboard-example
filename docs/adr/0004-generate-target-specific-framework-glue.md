...
Browser boot, hydration, client transport, per-page server dispatch, SSR handlers, page protocol codecs, route glue, and build metadata are generator-owned. User-owned modules should not be files that exist only to wrap target-specific generated plumbing.
