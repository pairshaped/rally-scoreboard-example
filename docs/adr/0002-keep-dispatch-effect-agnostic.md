...
The Generator Framework's root dispatch routes a Mount `ToServer` command to the one handler for that constructor. It receives the current `backend.Model`, `RequestContext`, and `ServerContext`, then returns `#(backend.Model, Effect(ToClient))`.
