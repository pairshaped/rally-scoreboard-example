...
The generator's server emissions are delivered as `ToClient` values. `ToServer` is the client-to-server command vocabulary. `ToClient` is the server-to-client result and event vocabulary.
