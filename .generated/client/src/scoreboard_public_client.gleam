...
fn initial_load(route: public_route.Route) -> Effect(Msg) {
  case route {
    ...
    public_route.SignIn | public_route.NotFound -> effect.none()
  }
}
...
fn route_page_init(route: public_route.Route) -> #(String, String) {
  case route {
    ...
    public_route.SignIn | public_route.NotFound -> #("NotFound", "null")
  }
}
...
fn view(model: Model) -> Element(Msg) {
  ...
  html.div([attribute.class("scoreboard-app")], [
    ...
    case model.route {
      ...
      public_route.SignIn -> sign_in_view(model.query)
      public_route.NotFound -> ui.not_found_view()
    },
  ])
}

fn sign_in_view(query: Dict(String, String)) -> Element(Msg) {
  let return_to = dict.get(query, "return_to") |> result.unwrap("")

  html.main([attribute.class("panel")], [
    html.h1([], [html.text("Sign In")]),
    sign_in_form(return_to),
  ])
}

fn sign_in_form(return_to: String) -> Element(Msg) {
  html.form(
    [
...
fn explainer(route route: public_route.Route) -> Element(Msg) {
  case route {
    ...
    public_route.SignIn ->
      ui.page_explainer("What this page exercises", [
        "Route: public-owned sign-in page at /sign_in.",
        "Load: renders the sign-in form without a WebSocket connection.",
  ...
  html.header([attribute.class("topbar")], [
    ...
    html.nav([attribute.class("nav")], [
      ...
      case signed_in {
        ...
        False ->
          nav_link(
            route: public_route.SignIn,
            label: "Sign In",
            active: route == public_route.SignIn,
          )
      },
